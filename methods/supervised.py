...
from utils.loss import FocalLoss, FocalLossV2
...
train_aug = v2.Compose(
            [
                ...
                v2.RandomErasing(p=0.3, scale=(0.02, 0.05), inplace=True),
            ]
        )
...
class SupervisedModel(L.LightningModule):
    def __init__(self, task: dict, cfg: dict, method="supervised_base"):
        ...
        self.ce_loss = FocalLossV2(alpha=1, gamma=0.25, ignore_index=NO_LABEL)
...
def main():
    ...
    model = SupervisedModel(tasks, cfg)

    trainer.fit(model=model, datamodule=data)
    # trainer.test(model, ckpt_path="/home/treerspeaking/src/python/cabdefect/lightning_logs/version_200/checkpoints/supervisedepoch=146-val/accuracy=1.0000-val/total_loss=0.005776646547019482.ckpt", datamodule=data)
